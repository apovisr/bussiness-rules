...
total_balance(john, -30).
total_balance(mary, -10).
total_balance(paul, -10).
total_balance(alex, -50).
total_balance(jane, 100).


suggest_payment(Person):-
    total_balance(Person, Total),
    wrapperIf(Person, Total).

wrapperIf(Person, Total):-
    Total < 0 -> find_positive(-1 * Total), format('to ~w.~n', [Person]);
    Total > 0 -> format('~w should pay.~n', [Person]), find_negative(-1 * Total).
...
find_negative(Total):-
    findall(Person-Amount, (total_balance(Person, Amount), Amount < 0), Debts),
    show_payments_negative(Total, Debts).
...
show_payments_negative(Total, [Person-Amount| T]):-
    -1 * Total > -1 * Amount ->  format('~d to ~w.~n', [-1 *Amount, Person]), show_payments_negative(Total - Amount, T), true;
    -1 * Total < -1 * Amount -> format('~d to ~w.~n', [-1 *Total, Person]);
    -1 * Total =:= -1 * Amount -> format('~d to ~w.~n', [-1 *Total, Person]).

% example
% suggest_payment(john).
% suggest_payment(jane).